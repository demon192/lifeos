...
- [x] **M2 — Understanding.** OCR text → Gemma 3 1B → `{amount, due_date}` LifeEvent JSON → display nicely. **✅ PIPELINE VERIFIED ON DEVICE (2026-09-14)** — first-run model download + on-device Gemma inference + result card all working on the iQOO Z7 Pro. **Capture UX (2026-09-14):** Capture takes a *still photo* → user confirms (**Retake / Use & understand**) → only on confirm does the app OCR the sharp still (`StillImageOcr`) and run Gemma. This replaced live-frame OCR for extraction because a steady, full-res still OCRs far better — accurate OCR text is the #1 driver of extraction quality. Extraction *quality* still being tuned (a 1B model; dense Indian electricity bills and photos-of-a-screen are hard — moiré/glare wreck OCR). Prompt hardened: field-location hints ("Total Payable"/"Bill Due Date"), and examples marked FORMAT-only so the model stops copying example values (it once echoed the example amount `1240`). **Demo target = a clean, well-lit English electricity bill (real paper beats a photo of a monitor).** Next quality lever if needed: low-temperature session API for determinism. **Crop + crash fix (2026-09-14):** added optional drag-to-crop on the frozen photo (dependency-free, clamped) so the user can isolate the bill. **The "app closes on Use & understand" crash is FIXED and its true cause is confirmed from logcat:** MediaPipe treats `setMaxTokens` as the *total* budget (input **+** output). A dense bill's prompt tokenized to **1294 tokens** while `MAX_TOKENS` was only **1280** — the input alone exceeded the whole budget, leaving nothing for a reply, so MediaPipe logged `OUT_OF_RANGE` ("Input is too long… input_size(1294) was not less than maxTokens(1280)") and then **died with a native SIGSEGV** inside `generateResponse` — a hard process death Kotlin's try/catch cannot catch. Two mistakes compounded: (a) `MAX_TOKENS` was too tight with no output headroom, and (b) the old char cap (2000) still let the input hit ~1294 tokens because **bills are number-dense and tokenize to far more tokens per character than prose** (the old "2000 chars ≈ 500–650 tokens" estimate was wrong for bills). Fix: raised `MAX_TOKENS` to **2048** and cut `MAX_OCR_CHARS` to **1200** — input now lands ~930 tokens with ~1100 free for the reply. Rule for future tuning: raise both together, never one alone. **OCR resolution (2026-09-14):** with the crash traced to tokens (not memory), the old 1600 px capture downscale — added as a memory-crash *guess* — was pure downside: it was shredding OCR of dense bills ("Fixed Demand Charges" → "Exed Demand Charges"), so `MAX_CAPTURE_DIM` was raised to **3000 px** (Gemma only ever gets the OCR *text*, never the bitmap, so higher res is free at inference). **Reality check on hard bills:** a dense, faded 2018 UP PVVNL bill photographed with the bill filling only ~half the frame is a torture test for on-device OCR. Demo strategy stands: clean, well-lit English bill, filling the frame (get close / go landscape), crop to isolate.
